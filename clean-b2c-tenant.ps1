...
foreach( $a in $resp.value ) {
    if ( !$a.displayName.StartsWith("b2c-extensions-app")) { # do NOT delete this app
        ...
        $b2cExtensionsAppId = $a.id
    }
}

# delete all extensionProperties for b2c-extension-app
$url = "https://graph.microsoft.com/beta/applications/$b2cExtensionsAppId/extensionProperties"
$resp = Invoke-RestMethod -Method GET -Uri $url -Headers $authHeader  
foreach( $g in $resp.value ) {
    $ret = Invoke-RestMethod -Method "DELETE" -Uri "$url/$($g.id)" -Headers $authHeader  
}
...
$url = "https://graph.microsoft.com/beta/applications?`$select=id,appId&`$filter=displayName eq '$($app.appName)'"
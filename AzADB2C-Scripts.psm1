...
.EXAMPLE
    Connect-AzADB2CEnv -t "yourtenant"

.EXAMPLE
    Connect-AzADB2CEnv -ConfigPath .\b2cAppSettings_yourtenant.json
#>
function Connect-AzADB2CEnv
(
    [Parameter(Mandatory=$false)][Alias('t')][string]$TenantName = "",
    ...
    )
{
                        ...
                        foreach( $tp in $cp.TechnicalProfiles ) {
                            ...
                            foreach( $ic in $tp.TechnicalProfile.InputClaims.InputClaim ) {
                                if ( "client_id" -eq $ic.ClaimTypeReferenceId ) {
                                    $ic.DefaultValue = $AppIdIEFProxy.ToString()
                                }
                                if ( "resource_id" -eq $ic.ClaimTypeReferenceId ) {
                                    $ic.DefaultValue = $AppIdIEF.ToString()
                                }
                            }
                        }
                    } else {
    ...
    if ( $True -eq $Clean ) {
        ...
        $AppIdIEF = (Get-AzADApplication -DisplayName $iefAppName).ApplicationId.Guid
        $AppIdIEFProxy = (Get-AzADApplication -DisplayName $iefProxyAppName).ApplicationId.Guid  
        if ( "" -ne $ExtAppDisplayName ) {    
            write-output "Getting AppID's for $ExtAppDisplayName"
            $appExt = Get-AzADApplication -DisplayNameStartWith $ExtAppDisplayName
            write-output $appExt.ApplicationId.Guid
            $appExtAppId = $appExt.ApplicationId.Guid
...
.EXAMPLE
    Get-AzADB2CPolicyId -PolicyId "B2C_1A_demo_TrustFrameworkExtensions"
#>
function Get-AzADB2CPolicyId
(
    [Parameter(Mandatory=$false)][Alias('p')][string]$PolicyId = "",
    [Parameter(Mandatory=$false)][Alias('f')][string]$PolicyFile = ""
    )
{
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "Policy.Read.TrustFramework"
    #write-host "Getting policy $PolicyId..."
    $url = "https://graph.microsoft.com/beta/trustFramework/policies/$PolicyId/`$value"
    $resp = Invoke-RestMethod -Method GET -Uri $url -ContentType "application/xml" -Headers $authHeader
    if ( "" -eq $PolicyFile ) {
...
#>
function List-AzADB2CPolicyIds()
{
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "Policy.Read.TrustFramework"
...
.EXAMPLE
    Deploy-AzADB2CPolicyToTenant

.EXAMPLE
    Deploy-AzADB2CPolicyToTenant -PolicyFile ".\SignUpOrSignin.xml"
#>
function Deploy-AzADB2CPolicyToTenant
(
    [Parameter(Mandatory=$false)][Alias('p')][string]$PolicyPath = "",
    [Parameter(Mandatory=$false)][Alias('f')][string]$PolicyFile = ""
)
{    
    ...
    Function UploadPolicy( [string]$PolicyId, [string]$PolicyData) {
        ...
        $url = "https://graph.microsoft.com/beta/trustFramework/policies/$PolicyId/`$value"
        try {
            $resp = Invoke-RestMethod -Method PUT -Uri $url -ContentType "application/xml" -Headers $authHeader -Body $PolicyData
    ...
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "Policy.ReadWrite.TrustFramework"
    if ( "" -ne $PolicyFile ) {
...
.EXAMPLE
    Delete-AzADB2CPolicyFromTenant

.EXAMPLE
    Delete-AzADB2CPolicyFromTenant -f ".\SignUpOrSignin.xml"
#>
function Delete-AzADB2CPolicyFromTenant
(
    [Parameter(Mandatory=$false)][Alias('p')][string]$PolicyPath = "",
    [Parameter(Mandatory=$false)][Alias('f')][string]$PolicyFile = "",
    [Parameter(Mandatory=$false)][Alias('i')][string]$PolicyId = ""
...
{    
    ...
    Function DeletePolicy( [string]$PolicyId) {
        ...
        $resp = Invoke-RestMethod -Method DELETE -Uri $url -Headers $authHeader
    }    
    if ( "" -eq $PolicyPath ) {
        $PolicyPath = (get-location).Path
    }
...
.EXAMPLE
    Set-AzADB2CClaimsProvider -ProviderName "Google"

.EXAMPLE
    Set-AzADB2CClaimsProvider -ProviderName "AzureAD" -AadTenantName "contoso.com"

.EXAMPLE
    Set-AzADB2CClaimsProvider -ProviderName "RESTAPI"
#>
function Set-AzADB2CClaimsProvider (
    [Parameter(Mandatory=$false)][Alias('p')][string]$PolicyPath = "",    
...
)
{
    ...
    $appIEF = New-AzADApplication -DisplayName $DisplayName -IdentifierUris "http://$TenantName/$DisplayName" -ReplyUrls @("https://$DisplayName") -RequiredResourceAccess $req1 # WebApp
    write-output "AppID`t`t$($appIEF.ApplicationId.Guid)`nObjectID:`t$($appIEF.ObjectID)"
    write-host "Creating ServicePrincipal..."
    $sp = New-AzADServicePrincipal -AccountEnabled $true -ApplicationId $appIEF.ApplicationId.Guid -AppRoleAssignmentRequired $false -DisplayName $DisplayName 
    write-host "AppID`t`t$($sp.ApplicationId.Guid)`nObjectID:`t$($sp.ObjectID)"

    $req2 = New-Object -TypeName "Microsoft.Open.AzureAD.Model.RequiredResourceAccess"
    $req2.ResourceAppId = $appIEF.ApplicationId.Guid
    ...
    $appPIEF = New-AzADApplication -DisplayName $ProxyDisplayName -ReplyUrls @("https://$ProxyDisplayName") -RequiredResourceAccess @($req1,$req2) -PublicClient $true # NativeApp
    write-output "AppID`t`t$($appPIEF.ApplicationId.Guid)`nObjectID:`t$($appPIEF.ObjectID)"
    write-host "Creating ServicePrincipal..."
    $sp = New-AzADServicePrincipal -AccountEnabled $true -ApplicationId $appPIEF.ApplicationId.Guid -AppRoleAssignmentRequired $false -DisplayName $ProxyDisplayName 
    write-host "AppID`t`t$($sp.ApplicationId.Guid)`nObjectID:`t$($sp.ObjectID)"

    Set-AzADB2CGrantPermissions -n $DisplayName
    Set-AzADB2CGrantPermissions -n $ProxyDisplayName
}
...
)
{
    ...
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "TrustFrameworkKeySet.Read.All,TrustFrameworkKeySet.ReadWrite.All"    
    $url = "https://graph.microsoft.com/beta/trustFramework/keySets"

    try {
        $resp = Invoke-RestMethod -Method GET -Uri "$url/$KeyContainerName" -Headers $authHeader -ErrorAction SilentlyContinue
...
"@
    $resp = Invoke-RestMethod -Method POST -Uri $url -Headers $authHeader -Body $body -ContentType "application/json" -ErrorAction SilentlyContinue
    <##>
...
.EXAMPLE
    Set-AzADB2CGrantPermissions -t "yourtenant" -n "Your-AppName"

#>
function Set-AzADB2CGrantPermissions
(
...
)
{
    ...
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "https://graph.microsoft.com/.default Directory.ReadWrite.All"    
    ...
    foreach( $reqResAccess in $app.RequiredResourceAccess ) { 
        ...
        Invoke-RestMethod -Uri $apiUrl -Headers $authHeader -Method POST -Body $($body | convertto-json) -ContentType "application/json"
    }
}
...
"@ | ConvertFrom-json
    ...
    $app = New-AzADApplication -DisplayName $DisplayName -IdentifierUris "https://$TenantName/$DisplayName" -ReplyUrls @("https://jwt.ms") -RequiredResourceAccess $reqAccess -Oauth2AllowImplicitFlow $true

    write-output "Creating ServicePrincipal $DisplayName"
    $sp = New-AzADServicePrincipal -AccountEnabled $true -ApplicationId $App.ApplicationId.Guid -AppRoleAssignmentRequired $false -DisplayName $DisplayName 
    # -----------------------------------------------------------------------------------------
    ...
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "https://graph.microsoft.com/.default Application.ReadWrite.All"    
    ...
    Invoke-RestMethod -Uri $apiUrl -Headers $authHeader -Method PATCH -Body ($body | ConvertTo-json) -ContentType "application/json"
...
.EXAMPLE
    Deploy-AzADB2CHtmlContent -f ".\html\unified.html" -a "yourstorageaccount" -p "containername/path1/path2" -k $stgkey

#>
function Deploy-AzADB2CHtmlContent (
...
"@ | ConvertFrom-json
    ...
    $app = New-AzADApplication -DisplayName $DisplayName -IdentifierUris "http://$TenantName/$DisplayName" -ReplyUrls @("https://$DisplayName") -RequiredResourceAccess $reqAccess
    write-output "AppID`t`t$($app.ApplicationId.Guid)`nObjectID:`t$($App.ObjectID)"
    ...
    $AppSecret = New-AzADApplicationPasswordCredential -ObjectId $App.ObjectID
    $AppSecretValue = $AppSecret.Value

    $env:B2CAppId=$App.ApplicationId.Guid
    $env:B2CAppKey=$AppSecretValue
    $global:B2CAppId=$App.ApplicationId.Guid
    $global:B2CAppKey=$AppSecretValue

    write-output "setting ENVVAR B2CAppID=$($App.ApplicationId.Guid)"
    $env:B2CAppId=$App.ApplicationId.Guid
    ...
    if ( $CreateConfigFile ) {
        ...
        $cfg.ClientCredentials.client_id = $App.ApplicationId.Guid
        ...
        `"ClientCredentials`": { `
            `"client_id`": `"$($App.ApplicationId.Guid)`", `
...
)
{
    ...
    $user = New-AzADUser -DisplayName $displayName -mailNickName $username -PasswordPolicies "DisablePasswordExpiration" `
    ...
    foreach( $roleName in $RoleNames) {
        $role = Get-AzADDirectoryRole | Where-Object {$_.displayName -eq $roleName}
        if ( $null -eq $role ) {
            $roleTemplate = Get-AzADDirectoryRoleTemplate | ? { $_.DisplayName -eq $roleName }
            $ret = Enable-AzADDirectoryRole -RoleTemplateId $roleTemplate.ObjectId        
            $role = Get-AzADDirectoryRole | Where-Object {$_.displayName -eq $roleName}
        }
        $ret = Add-AzADDirectoryRoleMember -ObjectId $role.ObjectId -RefObjectId $user.ObjectId
...
{
    $authHeader = Get-MSGraphAuthorizationHeader -Scope "https://graph.microsoft.com/.default Directory.Read.All"    
    $url = "https://graph.microsoft.com/beta/domains"
    $resp = Invoke-RestMethod -Method GET -Uri $url -ContentType "application/json" -Headers $authHeader 
    $B2CCustomDomains = @()
...
.EXAMPLE
    $tokens = Connect-AzADB2CDevicelogin -TenantId $global:TenantId -Scope "User.Read.All" 

.EXAMPLE
    $tokens = Connect-AzADB2CDevicelogin -TenantId $global:TenantId -Scope "Directory.ReadWrite.All" 
#>
...
Export-ModuleMember -function Connect-AzADB2CEnv
Export-ModuleMember -function Delete-AzADB2CPolicyFromTenant
Export-ModuleMember -function Deploy-AzADB2CHtmlContent
Export-ModuleMember -function Deploy-AzADB2CPolicyToTenant
...
Export-ModuleMember -function List-AzADB2CPolicyIds
...
Export-ModuleMember -function Set-AzADB2CClaimsProvider
...
Export-ModuleMember -function Set-AzADB2CGrantPermissions
